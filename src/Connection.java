...
class Connection extends Thread {
  ...
  public String joinRoom(String roomAddress, User user) {
    try {
      ...
      return ("Connected on " + roomAddress + "\nYou are ready to chatting! Enjoy it :)");
  ...
  public String listUsers() {
    if (users.size() != 0) return users.toString();
    return ("\nNo Users logged in");
  }
    ...
    try {
      ...
      do {
        ...
        switch (command) {
          ...
          case ":list_members":
            if (currentRoom != null) {
              toClient.writeUTF(currentRoom.listUsers());
              System.out.println("Listed members at room " + currentRoom.getAddress());
